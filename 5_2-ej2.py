...
# Datos del ejercicio
x_datos = np.array([200.0, 250.0, 300.0, 350.0, 400.0])
y_datos = np.array([30.0, 35.0, 40.0, 46.0, 53.0])
x_interp = 275.0
...
print(f"Eficiencia en T = {x_interp} °C: {y_interp:.4f} %")
print(f"Tiempo de procesamiento: {(fin - inicio) * 1000:.4f} ms")

# Gráfica
x_curva = np.linspace(200, 400, 100)
y_curva = [evaluar_newton(coeficientes, x_datos, xi) for xi in x_curva]

plt.figure(figsize=(8, 5))
plt.plot(x_curva, y_curva, label='Polinomio de Newton', color='orange')
plt.scatter(x_datos, y_datos, color='red', zorder=5, label='Datos originales')
plt.scatter(x_interp, y_interp, color='green', marker='x', s=100, zorder=6, label=f'Predicción en {x_interp} °C')
plt.title('Interpolación de Newton - Eficiencia del Motor')
